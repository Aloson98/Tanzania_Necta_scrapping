...
def main():
    # initiate Database
    connect("necta_database", host="localhost", port=27017)
    ...
    if homepage.status_code == 200:
        ...
        for school in list(reversed(links)):
            years = [2020, 2021, 2022,2023, 2024]
            
            print(f"Starting School: {school}")
            for year in years:
                url = f"https://onlinesys.necta.go.tz/results/{year}/acsee/{school}"
                ...
                if school_page.status_code == 200:
                    ...
                    try:
                        School.objects.get(school_name=school_name.strip())
                        SubjectPerformance.objects.get(school__school_name=school_name.strip(), result_year=year)
                    except:
                        ...
                        for data in datas:
                            ...
                            if "-" in content:
                                ...
                                if len(matches) > 0:
                                    ...
                                    student_subjects_results = StudentSubjectResults(school=school, data_index=data_index, result_year=year, **student_subjects)
                            ...
                            if "EXAMINATION CENTRE RANKING" in data.get_text() or "EXAMINATION CENTRE OVERALL PERFORMANCE" in data.get_text():
                                ...
                                total_passed_candidate = int(datas[table_index+4].contents[0].strip())
                                school_GPA = float(datas[table_index+6].contents[0].strip())
                            ...
                            if "EXAMINATION CENTRE SUBJECTS PERFORMANCE" in data.get_text():
                                ...
                                if year > 2022:
                                    ...
                                    while True:  # Ensure we don't go out of bounds
                                        ...
                                        if last_data_index + 1 >= len(datas):
                                            break
                    print(f"year: {year} - {school_name}: DONE")
